...
### 1 · fix Vast’s “[arch=amd64] only” sources ##!/usr/bin/env bash
###############################################################################
#  oblivion_host_amd64.sh · Head-less Steam-Link server for Vast.ai (AMD64-only)
#  – Removes all multi-arch / i386 steps                       (May 2025)
###############################################################################
set -euo pipefail
log(){ printf '\e[1;36m==> %s\n' "$*"; }
...
STEAM_USER="${STEAM_USER:-}"          # pass as Vast secret
STEAM_PASS="${STEAM_PASS:-}"          # pass as Vast secret
APPIDS=(2623190)                      # Oblivion GOTY (example)

### 1 · base runtime deps (64-bit only) #######################################
...
### 5 · pre-download game(s) (optional) #######################################
...
log "Use Steam-Link’s *Other Computer* flow, enter the PIN on this host once."
exit 0
##############################
